...
public class PayViewAction {
    ...
    */
    ...
    throws Exception {
      ...
      //  String remark1 = request.getParameter("remark1");
      //  String remark2 = CommUtil.null2String(request.getParameter("remark2"));
      //  String  remark2="goods";
      //  String  remark1="229531";
        
        
        //这一段都是自己添加的
        String opstate=request.getParameter("opstate");  //0：支付成功     -1 请求参数无效        -2 签名错误
        String orderid=request.getParameter("orderid");   //商户订单号(这个要把后四位去掉)  在保存的时候要去掉后四位
        String ovalue=request.getParameter("ovalue");     //订单金额
        String sysorderid=request.getParameter("sysorderid");   //海大付订单号
        String completiontime=request.getParameter("completiontime");   //海大付订单号
        String sign=request.getParameter("sign");   //MD5签名    32位小写MD5签名值，GB2312编码 (这个是从上一步返回过来的)
        String attach=request.getParameter("attach");  //或得orderid  //相当于remark1
        String keys="ea85cc48d41949b2864edb9336225e70";   //商户的密匙
        String str="orderid="+orderid+"&opstate="+opstate+"&ovalue="+ovalue+keys;
        
        //然后给str加上Md5加密。
	    String sign1 = Md5Encrypt.md5(str);
	    String remark1=attach.substring(0,6);  //这个相当于orderId
	    String remark2=attach.substring(6);    //这个相当于类型goods
	    String orderids=orderid.substring(0,19);  //这个相当于正确的订单号
	    
	    
	    System.out.println("支付成功 ================================"+opstate);
	    System.out.println("商户订单号================================"+orderid);
	    System.out.println("订单金额================================"+ovalue);
	    System.out.println("海大付订单号================================"+sysorderid);
	    System.out.println("海大付订单号================================"+completiontime);
	    System.out.println("MD5签名================================"+sign);
	    System.out.println("str加上Md5加密================================"+sign1);
	    System.out.println("orderId====================================="+remark1);
	    System.out.println("goods====================================="+remark2);
	    System.out.println("正确的订单号====================================="+orderids);
        ...
        IntegralGoodsOrder ig_order = null;
        if (remark2.equals("goods")){
        	//这个地方就是通过remark1.找到那个订单。
            order = this.orderFormService
            		.getObjById(CommUtil.null2Long(remark1));
            System.out.println("找到了那个订单");
     ...
     /*   String v_oid = request.getParameter("v_oid");
        ...
        String text = v_oid + v_pstatus + v_amount + v_moneytype + key;
        String v_md5text = Md5Encrypt.md5(text).toUpperCase();  */
        if(order.getOrder_id().equals(orderids)){
        	
        if (sign.equals(sign1)){
            if ("0".equals(opstate)){  //表示支付成功
            	System.out.println("快要保存数据了。");
                if (remark2.equals("goods")){
                    order.setOrder_status(20);
                    order.setPayTime(new Date());
                    System.out.println("订单的状态要改变了============"+order.getOrder_status());
                    this.orderFormService.update(order);
                    update_goods_inventory(order);
                    OrderFormLog ofl = new OrderFormLog();
                    ofl.setAddTime(new Date());
                    ...
                    ofl.setOf(order);
                    this.orderFormLogService.save(ofl);
                    System.out.println("往oderformlog里面放数据了。=========="+ofl.getOf());
                    mv.addObject("obj", order);
                ...
                response.getWriter().write(opstate);
            }
          }else{
				mv = new JModelAndView("error.html", this.configService.getSysConfig(),
					                      this.userConfigService.getUserConfig(), 1, request,
