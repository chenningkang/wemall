...
public class CartViewAction {
    ...
    public ModelAndView goods_cart1(HttpServletRequest request, HttpServletResponse response){
        ...
        if((wemall_view_type != null) && (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))){
            ...
            mv = new JModelAndView("goods_cart1.html", this.configService.getSysConfig(),
                    this.userConfigService.getUserConfig(), 1, request, response);
        }
        List<StoreCart> cart = cart_calc(request);
        if (cart != null){
    ...
    public ModelAndView goods_cart2(HttpServletRequest request, HttpServletResponse response, String store_id){
        ...
        if((wemall_view_type != null) && (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))){
        	...
        	mv = new JModelAndView("goods_cart2.html", this.configService.getSysConfig(),
                    this.userConfigService.getUserConfig(), 1, request, response);
        }
        List<StoreCart> cart = cart_calc(request);
        StoreCart sc = null;
    ...
    public ModelAndView goods_cart3(HttpServletRequest request, HttpServletResponse response, String cart_session,
        ...
        if((wemall_view_type != null) && (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))){
        	...
        	mv = new JModelAndView("goods_cart3.html", this.configService.getSysConfig(),
                    this.userConfigService.getUserConfig(), 1, request, response);
        }
        ...
        if (cart != null){
            if (CommUtil.null2String(cart_session1).equals(cart_session)){
                ...
                if((wemall_view_type != null) && (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))){
                    ...
                    new JModelAndView("error.html", this.configService.getSysConfig(),
                            this.userConfigService.getUserConfig(), 1, request, response);
                }
    ...
    public ModelAndView order_pay_view(HttpServletRequest request, HttpServletResponse response, String id){
        ...
        if((wemall_view_type != null) && (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))){
        	...
        	mv = new JModelAndView("order_pay.html", this.configService.getSysConfig(),
                    this.userConfigService.getUserConfig(), 1, request, response);
        }
        //这个地方是首先把定胆查取出来，然后根据定胆的状态，=10了，才可以支付。
        ...
        if (of.getOrder_status() == 10){
            if (CommUtil.null2String(payType).equals("")){
                ...
                if (this.configService.getSysConfig().getConfig_payment_type() == 1){
                    ...
                    //下面这一行是自己加的
                 //   params.put("bankId", bankId);  //这个地方是为了带值过去。
                ...
                if (payType.equals("balance")){// 余额支付
                    ...
                    mv.addObject("store_id",
                                 this.orderFormService.getObjById(CommUtil.null2Long(order_id)).getStore().getId());
                    mv.addObject("pay_session", pay_session);
                }else{// 在线支付    这里过去的时候要把bankId带上，不然不会走那个东西的。
                ...
                mv.addObject("order_id", order_id+bankId+"");
            }
        }else{
            if((wemall_view_type != null) && (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))){
                mv = new JModelAndView("wap/error.html", this.configService.getSysConfig(),
...
//                    mv.addObject("pay_session", pay_session);
                    ...
                    return "redirect:" + CommUtil.getURL(request) + "/payafter_wappay.htm?order_id=" + order_id;
                }else if (payType.equals("weixin_wap")){// 微信公众号支付
     ...
     * @return
     */
    @SecurityMapping(display = false, rsequence = 0, title = "货到付款", value = "/payafter_wappay.htm*", rtype = "buyer", rname = "货到付款", rcode = "goods_cart", rgroup = "在线购物")
    @RequestMapping({ "/payafter_wappay.htm" })
    public ModelAndView payafter_wappay(HttpServletRequest request, HttpServletResponse response, String order_id){
        ModelAndView mv = new JModelAndView("wap/payafter_pay.html", this.configService.getSysConfig(),
                this.userConfigService.getUserConfig(), 1, request, response);
        String pay_session = CommUtil.randomString(32);
        request.getSession(false).setAttribute("pay_session", pay_session);
        mv.addObject("paymentTools", this.paymentTools);
    ...
    public ModelAndView cart_address(HttpServletRequest request, HttpServletResponse response, String id, String store_id){
        ...
        if((wemall_view_type != null) && (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))){
        	...
        	mv = new JModelAndView("cart_address.html", this.configService.getSysConfig(),
                    this.userConfigService.getUserConfig(), 1, request, response);
        }
    ...
    public ModelAndView address(HttpServletRequest request, HttpServletResponse response, String currentPage, String orderBy, String orderType, String store_id){
        ...
        if((wemall_view_type != null) && (!wemall_view_type.equals("")) && (wemall_view_type.equals("wap"))){
        	...
        	mv = new JModelAndView("address.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
        }
