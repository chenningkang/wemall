...
public class PayTools {
    ...
    public String genericChinaBank(String url, String payment_id, String type, String id){
        ...
        IntegralGoodsOrder ig_order = null;
        //首先把id，拿到
        String bankId=id.substring(6);  //这个获取的是bankId
        String ids=id.substring(0, 6);   
      //  System.out.println(bankId);
       // System.out.println(ids);
        
        if (type.equals("goods")){
        	
            of = this.orderFormService.getObjById(CommUtil.null2Long(ids));  //通过id找到订单号
            System.out.println(of.getOrder_id());
            System.out.println(of.getId());
        }
        if (type.equals("cash")){
            obj = this.predepositService.getObjById(CommUtil.null2Long(ids));
        }
        if (type.equals("gold")){
            gold = this.goldRecordService.getObjById(CommUtil.null2Long(ids));
        }
        if (type.equals("integral")){
            ig_order = this.integralGoodsOrderService.getObjById(
                           CommUtil.null2Long(ids));
        }
        Payment payment = this.paymentService.getObjById(
                              CommUtil.null2Long(payment_id));
        if (payment == null)
        ...
        String v_mid = payment.getChinabank_account();  //商户Id
        String parter= v_mid;
        //  list.add(new SysMap("v_mid", v_mid));
        list.add(new SysMap("parter", parter));
        
        //然后添加一个type(银行标识)
        list.add(new SysMap("type",bankId));            //银行标识
        ...
        list.add(new SysMap("value", v_amount));        //表示金额
        ...
        //这个地方要给订单号后面添加4位随机数。然后在放list.
        String shu=getFourRandom();
        String dindanhao=null;
        dindanhao=v_oid+shu;
        list.add(new SysMap("orderid", dindanhao));    //商户订单号
        
        //添加下行地址
        String v_url = url + "/chinabanks_return.htm";
        ...
        //添加attach
     //   String attach=ids;
    //    list.add(new SysMap("attach",attach));  //这个地方放的就是orderid
        
        String attach = "";
        if (type.equals("goods")){
        	attach = of.getId().toString();
        }
        if (type.equals("cash")){
        	attach = obj.getId().toString();
        }
        if (type.equals("gold")){
        	attach = gold.getId().toString();
        }
        if (type.equals("integral")){
        	attach = ig_order.getId().toString();
        }
        
        String remark2 = type;  //这个attach里面携带了orderid和商品类型
        list.add(new SysMap("attach", attach+type));
       
        //list.add(new SysMap("remark2", remark2));
        
        
        
        String key = payment.getChinabank_key();        //商户密匙
        list.add(new SysMap("key", key));
        
      // String v_moneytype = "CNY";      //这地方就是把bankid要带过来
      //  list.add(new SysMap("v_moneytype", v_moneytype));
       // String temp = v_amount + v_moneytype + v_oid + v_mid + v_url + key;
        
        String temp="parter=" + parter+"&type=" +bankId+"&value=" + v_amount+"&orderid=" + dindanhao+"&callbackurl=" + v_url;
		
		String mD5UrlAll = (temp + key).toString();
		String sign = Md5Encrypt.md5(mD5UrlAll);
		
        list.add(new SysMap("sign", sign));
        
 /*       String v_url = url + "/chinabank_return.htm";
        String callbackurl=v_url;   //下行通知地址
        list.add(new SysMap("v_url", v_url));
        
        String v_oid = "";
        if (type.equals("goods")){
            v_oid = of.getOrder_id();
            String orderid= v_oid;                       //商户订单号
        }
        if (type.equals("cash")){
            v_oid = obj.getPd_sn();
        }
        ...
        //下面这个地方要把东西拿到/chinabanks_return.htm 这个方法里面。
   /*     String remark1 = "";
        if (type.equals("goods")){
